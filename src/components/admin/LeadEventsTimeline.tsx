...
import {
  UserCog, Building2, Globe, Lock, XCircle, CheckCircle2, PauseCircle,
  PlayCircle, Clock, Eye, PhoneCall, Ban, RefreshCw, Sparkles,
} from "lucide-react";
...
const EVENT_META: Record<string, { label: string; icon: React.ComponentType<{ className?: string }>; tone: string }> = {
  ...
  "assignment.contacted": { label: "Mover contacted customer", icon: PhoneCall, tone: "text-amber-700" },
  "assignment.declined": { label: "Mover declined", icon: Ban, tone: "text-rose-700" },
  "assignment.withdrawn": { label: "Broker withdrew assignment", icon: RefreshCw, tone: "text-slate-700" },
  "assignment.claimed": { label: "Mover claimed open lead", icon: Sparkles, tone: "text-emerald-700" },
...
function summarize(e: LeadEvent): string | null {
  const p = e.payload ?? {};
  if (e.event_type === "assignment.invited" && typeof p.sla_hours === "number") return `SLA ${p.sla_hours}h`;
  ...
  if (e.event_type === "assignment.declined" && typeof p.reason === "string") return String(p.reason);
  return null;
}
...
export function LeadEventsTimeline({ quoteId }: { quoteId: string }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "lead_events", filter: `quote_id=eq.${quoteId}` },
  ...
  if (events.length === 0) return <p className="text-sm text-muted-foreground">No lead events yet.</p>;
      ...
      {events.map((e) => {
        ...
        return (
          <li key={e.id} className="ml-4 relative">
            ...
            <div className="text-sm font-medium">{meta.label}
              {extra && <span className="ml-2 text-xs font-normal text-muted-foreground">· {extra}</span>}
            </div>
