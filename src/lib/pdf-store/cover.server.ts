...
export function coverPrompt(title: string, category: string, spec: CoverSpec) {
  ...
  return [
    ...
    `Flat vector illustration, generous negative space, portrait 4:5 composition, print-ready.`,
...
export async function generateCover(
  ...
  if (!png) return { ok: false, cover_url: null, cover_status: "fallback", prompt };
  ...
  if (error) {
    console.error("[pdf-cover] upload failed:", error.message);
    return { ok: false, cover_url: null, cover_status: "fallback", prompt };
  }

  return { ok: true, cover_url: coverPublicPath(product.slug), cover_status: "generated", prompt };
}

/**
 * Backfill covers for products that still render the SVG fallback.
 * Used by the autonomous worker tick and by the admin console.
 */
export async function backfillCovers(db: any, limit: number) {
  ...
  for (const p of rows) {
    ...
    if (result.ok) {
      generated += 1;
      await db
        .from("pdf_publish_log")
        .insert({ product_slug: p.slug, action: "cover", detail: "Cover image generated" });
    }
  }
