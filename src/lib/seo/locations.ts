...
export const STATES: Location[] = [
  ...
  ["vermont","Vermont","VT"],["virginia","Virginia","VA"],["washington-state","Washington","WA"],
  ["west-virginia","West Virginia","WV"],["wisconsin","Wisconsin","WI"],["wyoming","Wyoming","WY"],
].map(([slug, name, code]) => ({ slug, name, kind: "state" as const, state: code, stateName: name }));
...
export const CITIES: Location[] = [
  ...
  slug: slug as string, name: name as string, kind: "city" as const,
  state: state as string, stateName: stateName as string, population: pop as number,
}));
