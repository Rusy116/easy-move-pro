...
import { BarChart3, TrendingUp, Clock, DollarSign, Target, XCircle, Truck, Award } from "lucide-react";
import {
...
function AnalyticsPage() {
  ...
  const kpis = useMemo(() => {
    ...
    const won = rows.filter((r) => r.assignment && ["accepted", "won"].includes(r.assignment.state));
    const lost = rows.filter((r) => r.assignment && ["lost", "declined", "expired", "withdrawn"].includes(r.assignment.state));
    const quoted = rows.filter((r) => r.assignment && ["quoted", "accepted", "won"].includes(r.assignment.state));
    const revenue = won.reduce((s, r) => s + Number(r.assignment?.quoted_amount ?? r.lead.estimated_high), 0);
    ...
    const estimateAcceptance = quoted.length ? (won.filter((r) => r.assignment && ["accepted", "won"].includes(r.assignment.state)).length / quoted.length) * 100 : 0;
    ...
    const responses = rows
      ...
      .map((r) => (new Date(r.assignment!.contacted_at!).getTime() - new Date(r.assignment!.invited_at).getTime()) / 60000);
    ...
    const inLastDays = (days: number) =>
      won.filter((r) => {
        const t = r.lead.move_date ? new Date(r.lead.move_date).getTime() : new Date(r.lead.created_at).getTime();
        return t >= now - days * 86400000 && t <= now + days * 86400000;
      }).length;
    const jobsThisWeek = inLastDays(7);
    const jobsThisMonth = inLastDays(30);
    const activeCrews = won.filter((r) => r.lead.move_date && new Date(r.lead.move_date) >= new Date()).length;

    return {
      total, won: won.length, lost: lost.length, revenue, avgJob, winRate, closeRate,
      estimateAcceptance, conversion, cancellationRate, avgResponse, jobsThisWeek, jobsThisMonth, activeCrews,
    };
  }, [merged]);

  const trends = useMemo(() => {
    const buckets: Array<{ day: string; leads: number; jobs: number; revenue: number; estimates: number }> = [];
    for (let i = 29; i >= 0; i--) {
      const d = daysAgo(i);
  ...
  const stats: Array<{ label: string; value: string; icon: React.ReactNode; hint?: string }> = [
    { label: "Revenue (won)", value: `$${Math.round(kpis.revenue).toLocaleString()}`, icon: <DollarSign className="h-4 w-4" /> },
    { label: "Win rate", value: `${kpis.winRate.toFixed(1)}%`, icon: <Award className="h-4 w-4" />, hint: `${kpis.won} of ${kpis.total}` },
    { label: "Lead conversion", value: `${kpis.conversion.toFixed(1)}%`, icon: <TrendingUp className="h-4 w-4" />, hint: "Quoted / total" },
    { label: "Response time", value: `${Math.round(kpis.avgResponse)} min`, icon: <Clock className="h-4 w-4" /> },
    { label: "Avg job value", value: `$${Math.round(kpis.avgJob).toLocaleString()}`, icon: <DollarSign className="h-4 w-4" /> },
    { label: "Close rate", value: `${kpis.closeRate.toFixed(1)}%`, icon: <Target className="h-4 w-4" />, hint: "Won / quoted" },
    { label: "Active crews", value: kpis.activeCrews.toString(), icon: <Truck className="h-4 w-4" /> },
    { label: "Jobs this week", value: kpis.jobsThisWeek.toString(), icon: <BarChart3 className="h-4 w-4" /> },
    { label: "Jobs this month", value: kpis.jobsThisMonth.toString(), icon: <BarChart3 className="h-4 w-4" /> },
    { label: "Estimate acceptance", value: `${kpis.estimateAcceptance.toFixed(1)}%`, icon: <Award className="h-4 w-4" /> },
    { label: "Cancellation rate", value: `${kpis.cancellationRate.toFixed(1)}%`, icon: <XCircle className="h-4 w-4" /> },
    { label: "Total leads", value: kpis.total.toString(), icon: <TrendingUp className="h-4 w-4" /> },
  ];
              ...
              return (
                <div key={r.reason}>
                  ...
                  <div className="h-2 rounded bg-muted overflow-hidden mt-1">
                    <div className="h-full bg-primary/70" style={{ width: `${(r.count / max) * 100}%` }} />
                  </div>
                </div>
              );
            })}
