...
function InvoicesPage() {
  ...
  async function downloadPdf(inv: Invoice) {
    ...
    doc.text(
      `Tax (${Number(inv.tax_rate).toFixed(2)}%): $${Number(inv.tax_amount).toFixed(2)}`,
      140,
      finalY + 12,
    );
    doc.setFontSize(12);
              ...
              {filtered.map((r) => (
                <tr key={r.id} className="border-b border-border/70">
                  ...
                  <td className="py-3 flex items-center gap-1">
                    ...
                    {r.status === "draft" && (
                      <Button variant="ghost" size="sm" onClick={() => void markSent(r)}>
                        Send
                      </Button>
                    )}
                    ...
                    <Button variant="ghost" size="sm" onClick={() => void del(r)} title="Delete">
...
function InvoiceDialog({
  ...
  async function save() {
    ...
    if (invoice) {
      ...
      // number is generated by trigger when empty
      const { data, error } = await supabase
        .from("company_invoices")
        .insert({ ...payload, number: "", created_by: u.user?.id ?? null })
        .select()
        .single();
      if (error || !data) {
    ...
    if (id) {
      ...
      await supabase.from("company_invoice_items").insert(
        items.map((it, idx) => ({
          invoice_id: id,
          company_id: company,
          description: it.description,
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <select
                ...
                value={kind}
                onChange={(e) => setKind(e.target.value as Invoice["kind"])}
              >
            ...
            {items.map((it, idx) => (
              <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                <Input
                  ...
                  onChange={(e) =>
                    setItems((a) =>
                      a.map((x, i) => (i === idx ? { ...x, description: e.target.value } : x)),
                    )
                  }
                />
                <Input
                  ...
                  onChange={(e) =>
                    setItems((a) =>
                      a.map((x, i) => (i === idx ? { ...x, quantity: Number(e.target.value) } : x)),
                    )
                  }
                />
                <Input
