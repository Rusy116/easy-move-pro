...
function MessagesPage() {
  ...
  const loadConvs = useCallback(async () => {
    if (!company) return;
    const { data } = await supabase.from("company_conversations")
  ...
  const loadMsgs = useCallback(async (id: string) => {
    const { data } = await supabase.from("company_messages")
      .select("*").eq("conversation_id", id).order("created_at");
    setMsgs((data as Message[] | null) ?? []);
    void supabase.from("company_messages").update({ read_at: new Date().toISOString() })
      .eq("conversation_id", id).is("read_at", null);
  }, []);
  ...
  useEffect(() => {
    if (!company) return;
    const ch = supabase.channel(`co-msgs-${company.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "company_messages", filter: `company_id=eq.${company.id}` },
        () => { void loadConvs(); if (activeId) void loadMsgs(activeId); })
      .on("postgres_changes", { event: "*", schema: "public", table: "company_conversations", filter: `company_id=eq.${company.id}` },
        () => { void loadConvs(); })
      .subscribe();
  ...
  async function createConversation() {
    ...
    const { data: u } = await supabase.auth.getUser();
    const { data: conv, error } = await supabase.from("company_conversations").insert({
      company_id: company.id, kind: newKind, subject: newSubject.trim(), created_by: u.user?.id ?? null,
    }).select().single();
    ...
    await supabase.from("company_messages").insert({
      ...
      sender_user_id: u.user?.id ?? null, sender_role: "mover",
      sender_name: u.user?.email ?? "You", body: newBody.trim(),
    });
    toast.success("Conversation started");
    setOpen(false); setNewSubject(""); setNewBody("");
    await loadConvs();
    setActiveId(conv.id);
  }
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,340px)_minmax(0,1fr)] h-[calc(100vh-260px)] min-h-[520px]">
        <div className="card-premium p-3 flex flex-col min-h-0">
          <div className="flex items-center justify-between gap-2 mb-2">
            <div className="flex items-center gap-2 px-1"><MessageSquare className="h-4 w-4 text-primary" /><h2 className="font-serif text-lg">Inbox</h2></div>
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                ...
                <div className="space-y-3">
                    ...
                    {(["broker", "internal"] as const).map((k) => (
                      <button key={k} type="button" onClick={() => setNewKind(k)}
                        ...
                        <div className="flex items-center gap-2 text-sm font-medium capitalize">
                          {k === "broker" ? <ShieldCheck className="h-4 w-4" /> : <Users className="h-4 w-4" />}{k}
                        </div>
                  ...
                  <div className="space-y-1.5"><Label>Subject</Label><Input value={newSubject} onChange={(e) => setNewSubject(e.target.value)} placeholder="e.g. Question about lead EM-2026-000123" /></div>
                  <div className="space-y-1.5"><Label>Message</Label><Textarea rows={5} value={newBody} onChange={(e) => setNewBody(e.target.value)} /></div>
                </div>
                <DialogFooter><Button onClick={() => void createConversation()}>Start</Button></DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
          ...
          <div className="relative mb-2">
            ...
            <Input placeholder="Search conversations…" className="pl-9" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-1 -mx-1 px-1">
            ...
            {filtered.map((c) => (
              <button key={c.id} onClick={() => setActiveId(c.id)}
                ...
                <div className="text-[11px] text-muted-foreground mt-0.5">{new Date(c.last_message_at).toLocaleString()}</div>
              </button>
            ))}
          </div>
        </div>
            ...
            <>
              <div className="flex items-center justify-between gap-2 pb-3 border-b border-border">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground capitalize">{active.kind} conversation</div>
                </div>
                <Badge variant="outline" className="capitalize">{active.kind}</Badge>
              </div>
              ...
              <div ref={scrollerRef} className="flex-1 overflow-y-auto py-4 space-y-3">
                ...
                {msgs.length === 0 && <div className="text-center text-sm text-muted-foreground">No messages yet.</div>}
              </div>

              <div className="border-t border-border pt-3 flex items-end gap-2">
                <Textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Type a message…" rows={2} className="resize-none" />
                <Button onClick={() => void send()} disabled={sending || !draft.trim()}><Send className="h-4 w-4" /></Button>
              </div>
            </>
          )}
        </div>
