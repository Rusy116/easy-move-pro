...
type Notification = {
  id: string; company_id: string; type: string; title: string; body: string | null;
  payload: Record<string, unknown>; quote_id: string | null; read_at: string | null; created_at: string;
};

const FILTERS = ["all", "unread", "new_lead", "exclusive", "marketplace", "estimate", "schedule", "invoice", "broker"] as const;
...
function iconClass(type: string) {
  if (type.includes("estimate_accepted") || type.includes("invoice_paid") || type.includes("won")) return "bg-emerald-50 text-emerald-700 border-emerald-300";
  if (type.includes("rejected") || type.includes("lost") || type.includes("expired")) return "bg-rose-50 text-rose-700 border-rose-300";
...
function NotificationsPage() {
  ...
  const load = useCallback(async () => {
    if (!company) return;
    const { data } = await supabase.from("company_notifications").select("*").eq("company_id", company.id).order("created_at", { ascending: false }).limit(200);
  ...
  useEffect(() => {
    if (!company) return;
    const ch = supabase.channel(`co-notif-${company.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "company_notifications", filter: `company_id=eq.${company.id}` },
        () => void load())
      .subscribe();
  ...
  async function markAllRead() {
    if (!company) return;
    const { error } = await supabase.from("company_notifications").update({ read_at: new Date().toISOString() }).eq("company_id", company.id).is("read_at", null);
    if (error) toast.error(error.message); else { toast.success("All marked read"); void load(); }
  }
  async function markRead(id: string) {
    await supabase.from("company_notifications").update({ read_at: new Date().toISOString() }).eq("id", id);
    void load();
  }
    ...
    <div className="space-y-6">
      ...
      <div className="card-premium p-4 md:p-5 space-y-4">
        ...
        <div className="space-y-2">
          ...
          {filtered.map((n) => (
            <div key={n.id} className={`rounded-lg border p-4 flex items-start gap-3 ${!n.read_at ? "border-primary/30 bg-primary/5" : "border-border bg-card"}`}>
              <div className={`h-8 w-8 shrink-0 rounded-full border grid place-items-center ${iconClass(n.type)}`}>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {!n.read_at && <Badge className="bg-primary text-primary-foreground text-[10px]">New</Badge>}
                </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                {!n.read_at && <Button size="sm" variant="ghost" onClick={() => void markRead(n.id)}>Mark read</Button>}
                <Button size="sm" variant="ghost" onClick={() => void remove(n.id)}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
          ))}
        </div>
