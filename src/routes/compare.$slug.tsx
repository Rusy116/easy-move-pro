...
import {
  SeoHero, ComparisonTable, Faq, Cta, Breadcrumbs, InternalLinks,
  ...
  return (
    <SiteLayout>
      ...
      <InternalLinks
        ...
        links={COMPARISON_PAGES.filter((p) => p.slug !== page.slug).map((p) => ({ label: `Easy Moving vs ${p.competitor}`, to: `/compare/${p.slug}` }))}
      />
      <Cta />
    </SiteLayout>
  );
