...
import {
  SeoHero, FeatureGrid, Faq, Cta, Breadcrumbs, InternalLinks, Statistics,
...
function buildFaq(name: string, isState: boolean) {
  return [
    { q: `How do I get moving leads in ${name}?`, a: `Apply to become an Easy Moving partner. Once approved, you'll get access to exclusive and marketplace ${name} moving leads with verified customer details.` },
    { q: `How much do ${name} moving leads cost?`, a: `Pricing depends on move type and demand. Local ${isState ? "moves in " + name : name + " moves"} start around $40 for exclusive leads. Marketplace leads are lower.` },
    { q: `What areas do you cover in ${name}?`, a: `We cover every metro in ${name}. Set your specific service ZIPs, cities, and radius during onboarding.` },
    { q: "How fast can I start receiving leads?", a: "1–3 business days after verification of DOT/MC and insurance." },
  ];
}
  ...
  return (
    <SiteLayout>
      <div className="mx-auto max-w-3xl px-4 sm:px-6 py-24 text-center">
        ...
        <Link to="/partners" className="mt-6 inline-block text-primary underline">Back to partners</Link>
      </div>
    </SiteLayout>
  );
}
...
function LocationPage() {
  ...
  return (
    <SiteLayout>
      <Breadcrumbs items={[{ label: "Home", to: "/" }, { label: "Partners", to: "/partners" }, { label: location.name }]} />
      <SeoHero
      ...
      <section className="border-b border-border">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 py-16 prose prose-neutral dark:prose-invert">
          ...
          <p className="mt-4 text-base leading-relaxed text-muted-foreground">
            Every ${location.name} partner also gets our built-in CRM, estimate builder, dispatch board, and invoicing — replacing 5+ tools you'd otherwise stitch together. Pay only for the leads you claim; no monthly minimum, no long-term contracts.
          </p>
        </div>
      </section>
      ...
      <FeatureGrid
        ...
        items={[
          { title: `${where} exclusive leads`, body: "12-hour claim window before a lead opens to the marketplace." },
          { title: "Verified customer info", body: "Every phone and email is validated before it reaches you." },
          { title: "Full move details", body: "Cubic feet, inventory, distance, dates — never just a name." },
          { title: "Built-in CRM & dispatch", body: "Manage the full lifecycle from lead to invoice on one platform." },
          { title: "No monthly minimum", body: "Pay per lead. Pause any time. Scale up or down freely." },
          { title: "Refund on bad leads", body: "Duplicate or bogus number? We credit your account within 48 hours." },
        ]}
      />
      ...
      {cities.length > 0 && (
        <section className="border-b border-border">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 py-16">
            <h2 className="font-serif text-2xl md:text-3xl font-semibold tracking-tight">Cities we cover in {location.name}</h2>
            <div className="mt-6 grid gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              {cities.map((c) => (
                <Link key={c.slug} to="/partners/$location" params={{ location: c.slug }} className="rounded-lg border border-border bg-card px-3 py-2 text-sm hover:border-primary hover:text-primary transition-colors">
                  {c.name}
                </Link>
              ))}
            </div>
      ...
      {!isState && (
        <InternalLinks
          ...
          links={CITIES
            .filter((c) => c.state === location.state && c.slug !== location.slug)
            .slice(0, 10)
