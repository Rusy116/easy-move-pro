...
BEGIN
  ...
  SET accepted_at = COALESCE(q.accepted_at, now()),
      ...
      lead_phase = CASE WHEN q.status IN ('won','lost','cancelled') THEN q.lead_phase ELSE 'closed'::lead_phase_enum END,
      closed_at = CASE WHEN q.status IN ('won','lost','cancelled') THEN q.closed_at ELSE COALESCE(q.closed_at, now()) END,
      closed_reason = CASE WHEN q.status IN ('won','lost','cancelled') THEN q.closed_reason ELSE COALESCE(q.closed_reason, 'customer_accepted') END,
